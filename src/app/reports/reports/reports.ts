...
import { ReportExportService, ColumnDef } from './report-export.service';

type Row = { id: number; name: string; amount: number; date: string };
...
export class ReportsComponent {
  // header
  cols: ColumnDef<Row>[] = [
    { key: 'id', header: '#', align: 'right', width: 8 },
    { key: 'name', header: 'Name', align: 'left', width: 22 },
    { key: 'amount', header: 'Amount', align: 'right', width: 14 },
    { key: 'date', header: 'Date', align: 'left', width: 16 },
  ];

  // data
  // rows: Row[] = [
  //   { id: 1, name: 'Alice', amount: 1200.5, date: '2025-09-05' },
  //   { id: 2, name: 'Bob', amount: 300.0, date: '2025-09-03' },
  // ];

  rows: Row[] = Array.from({ length: 100 }, (_, i) => {
    const id = i + 1;
    const name = `ម៉េង User ${id}`;
  ...
  exportPDF() {
    this.exportService.exportPdf(this.rows, this.cols, {
      ...
      signatures: {
        ...
        // names: ['Meng', '', 'Director'],        // optional presets
        // positions: ['Accountant', '', ''],      // optional presets
        showDate: true,
      },
  ...
  exportXLSX() {
    this.exportService.exportExcel(this.rows, this.cols, {
      title: 'Sales Report',
      subtitle: 'Branch: Phnom Penh',
      signatures: {
        titles: ['Prepared By', 'Checked By', 'Approved By'],
  ...
  exportCSV() {
    this.exportService.exportCsv(this.rows, this.cols, {
      title: 'Sales Report',
      filename: 'sales-report',
    });
  }
